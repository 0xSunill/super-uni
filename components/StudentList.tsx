...
export default function StudentList({ data = [] as ClientStudent[],
  ...
  const [students, setStudents] = useState<ClientStudent[]>(data);
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter((s) => `${s.name} ${s.email}  ${s.rollNo}`.toLowerCase().includes(q));
  }, [students, query]);
  ...
  function handleAdd(e?: React.FormEvent) {
    ...
    // setOpenAdd(false);
    // setPage(1);
    alert("Add student not implemented in this demo.");
  }

  function onDelete(s: ClientStudent) {
    // if (!confirm(`Delete ${s.name}?`)) return;
    // setStudents((slist) => slist.filter(x => x.id !== s.id));
    alert("Delete student not implemented in this demo.");
  }
        ...
        {current.map((s) => (
          <div key={s.id} className="bg-slate-50 dark:bg-[#000000] rounded-lg p-3 flex items-start justify-between">
            ...
            <div className="flex flex-col items-end gap-2">
              <div className="text-sm font-semibold">{s.rollNo}</div>
